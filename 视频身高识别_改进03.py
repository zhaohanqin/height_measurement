    ...
    if os.path.exists(reference_face_file):
        try:
            ...
            if len(faces) > 0:
                ...
                if descriptors is not None and len(descriptors) > 0:
                    ...
                    print("⚠️ 无法从参考图片中提取人脸特征")
            else:
                print("⚠️ 在参考图片中未检测到人脸")
        except Exception as e:
            print(f"❌ 处理参考图片时出错: {e}")
    else:
        ...
        try:
            ...
            while True:
                ...
                # 显示预览
                cv2.imshow("Take Reference Photo (Press SPACE to capture, ESC to cancel)", frame)
            ...
            if os.path.exists(reference_face_file):
                ...
                if len(faces) > 0:
                    ...
                    if descriptors is not None and len(descriptors) > 0:
                        ...
                        print("⚠️ 无法从新拍摄的照片中提取人脸特征")
                else:
                    print("⚠️ 在新拍摄的照片中未检测到人脸")
        except Exception as e:
            print(f"❌ 拍照过程出错: {e}")
